...
namespace vor {
    ...
    std::vector<std::size_t>& Grid::operator()(std::size_t x, std::size_t y) {
        return m_cells[y * m_width + x]; // access the element at the given x and y coordinates
    }

    const std::vector<std::size_t>& Grid::operator()(std::size_t x, std::size_t y) const {
        return m_cells[y * m_width + x]; // access the element at the given x and y coordinates
    }
    ...
    // BoolArray2D implementation
    BoolArray2D::BoolArray2D(int width, int height) : width(width), height(height) {
    ...
    bool& BoolArray2D::operator()(int row, int col) {
